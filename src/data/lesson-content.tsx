...
sudo npm install -g @anthropic-ai/claude-code`}
      ...
      <Tip>
        <p>
          <strong>설치 명령어는 바뀔 수 있습니다.</strong> 위 명령어가 동작하지 않으면{" "}
...
{`방금 분석한 내용을 "연차조항_분석결과.md" 파일로 저장해줘`}
      ...
      <p>
        Claude Code에게 일을 시킬 때, <strong>어떻게 말하느냐</strong>에 따라 결과가
        완전히 달라집니다. 복잡한 기술이 아닙니다. 세 가지만 기억하세요.
      </p>

      <h3>원칙 1: 구체적으로 말하기</h3>
      <CodeBlock language="bash" filename="❌ 나쁜 예시 → ✅ 좋은 예시">
{`# ❌ 나쁜 예시: 너무 모호함
claude "이 파일 분석해줘"

# ✅ 좋은 예시: 무엇을, 어떻게, 어떤 형식으로
claude "취업규칙.docx에서 연차휴가 관련 조항을 찾아서, 근로기준법 제60조와 비교하고, 위반 사항이 있으면 표로 정리해줘"`}
      </CodeBlock>

      <h3>원칙 2: 출력 형식 지정하기</h3>
      <CodeBlock language="bash" filename="출력 형식 지정">
{`# ❌ 형식 미지정: 결과가 장문의 텍스트로 나옴
claude "급여대장을 검토해줘"

# ✅ 형식 지정: 깔끔한 표로 정리됨
claude "급여대장.xlsx를 검토하고, 오류가 있는 직원을 다음 형식의 표로 정리해줘:
| 이름 | 항목 | 현재값 | 올바른값 | 차이 |"`}
      </CodeBlock>

      <h3>원칙 3: 맥락 제공하기</h3>
      <CodeBlock language="bash" filename="맥락 제공">
{`# ❌ 맥락 없음: Claude가 추측해야 함
claude "이 계약서 검토해줘"

# ✅ 맥락 제공: 무엇을 중점적으로 볼지 명확
claude "이 근로계약서를 검토해줘. 나는 노무사고, 의뢰인이 부당해고를 주장하고 있어.
...
결과를 "문서목록_요약.md" 파일로 저장해줘.`}
      ...
      <CodeBlock language="bash" filename="프롬프트 개선 3단계">
{`# 1단계: 일단 시도
claude "취업규칙.docx를 검토해줘"
# → 결과가 너무 일반적이고 길다

# 2단계: 구체화
claude "취업규칙.docx에서 근로기준법 위반 가능성이 있는 조항만 찾아줘"
# → 좋지만, 어떤 기준인지 불명확

# 3단계: 기준 명시
claude "취업규칙.docx에서 다음 항목을 근로기준법 기준으로 검토해줘:
1. 연차휴가 (제60조)
2. 해고 절차 (제26-27조)
3. 근로시간 (제50-53조)
...
3. 결과를 "환불_거래내역.csv" 새 파일로 저장해줘`}
      ...
      <Warning>
        ...
        <ol className="space-y-1">
          ...
          <li>✅ <strong>절사/반올림 규칙</strong> — 보험료는 원 단위 절사(Math.floor), 수당은 반올림(Math.round)인지 확인</li>
...
Claude Desktop App → 설정 → Connectors → 원하는 서비스 클릭`}
      ...
      <Tip>
        <p>
          <strong>Desktop App이라면 &quot;커넥터(Connectors)&quot; 메뉴를 이용하세요.</strong>{" "}
          코드 입력 없이 GUI에서 클릭만으로 MCP를 연결할 수 있습니다.
          Gmail, Google Calendar, Notion 등 인기 서비스는 Connectors에서
          원클릭으로 연결되므로, CLI 명령어보다 훨씬 간편합니다.
          설정 경로: <code>Claude Desktop App → 설정 → Connectors</code>
        </p>
      </Tip>
...
Week 4: 자동화
  ...
  ✅ 전체 워크플로우 점검`}
      ...
      <CodeBlock language="bash" filename="사건 폴더 생성 및 문서 분석">
{`# 사건 폴더에서 Claude 실행
cd ~/사건/2026-가-1234_A사_부당해고
claude

# 의뢰인이 보낸 서류 일괄 분석
> "이 폴더의 모든 문서를 읽고 사건 개요를 정리해줘.
...
# 결과를 사건분석.md로 자동 저장`}
      ...
      <h3>2단계: 취업규칙 검토 (Phase 2 활용)</h3>
      <CodeBlock language="bash" filename="취업규칙 법령 대조">
{`# 취업규칙의 해고 절차가 근로기준법을 준수하는지 검토
> "취업규칙.docx에서 징계·해고 관련 조항을 모두 찾아서
   근로기준법 제23조(정당한 이유), 제26조(해고예고), 제27조(서면통지)와
   대조표를 만들어줘"

# 의견서 초안 작성
> "위 분석을 바탕으로 구제신청 이유서 초안을 작성해줘.
   IRAC 구조(쟁점-규범-적용-결론)로 작성하고,
   사실관계는 의뢰인 진술서 내용만 사용해"`}
      </CodeBlock>

      <h3>3단계: CLAUDE.md로 품질 보장 (Phase 3 활용)</h3>
      <CodeBlock language="markdown" filename="CLAUDE.md">
{`# 사건: 2026-가-1234 A사 부당해고

## 사건 개요
- 의뢰인: 홍길동 (A사 생산직, 3년 근무)
- 해고일: 2026.02.15
- 해고사유: 업무태만 (지각 3회)
- 쟁점: 해고의 정당성, 절차적 하자
...
## 작성 규칙
- 의견서는 IRAC 구조 사용
- 사실관계는 의뢰인 진술만 사용 (임의 추가 금지)
- 판례 인용 시 반드시 대법원 사건번호 명시
- 수치(퇴직금, 임금 등)는 검산 결과 병기`}
      </CodeBlock>

      <h3>4단계: 스킬로 자동화 (Phase 4 활용)</h3>
      <CodeBlock language="bash" filename="커스텀 스킬 실행">
{`# /사건분석 스킬로 한 번에 실행
/사건분석 "이 폴더의 사건 서류를 분석하고 구제신청 이유서 초안을 작성해줘"

# 이 스킬은 내부적으로:
# 1. 모든 문서를 읽고 사건 개요 정리
# 2. 취업규칙 vs 근로기준법 대조
# 3. IRAC 구조로 이유서 초안 작성
# 4. 결과를 이유서_초안.md로 저장`}
      </CodeBlock>

      <h3>5단계: 판례 검색 자동화 (Phase 5 활용)</h3>
      <CodeBlock language="bash" filename="Chrome MCP로 판례 검색">
{`# Claude가 브라우저를 열어 판례 검색
> "부당해고 관련 대법원 판례를 검색해줘.
...
# → 사건번호, 법원, 판시사항, 시사점`}
      ...
      <h3>6단계: 자동 실행 (Phase 6 활용)</h3>
      <CodeBlock language="bash" filename="비대화형 배치 처리">
{`# 퇴직금 자동 계산
claude -p "이 사건의 퇴직금을 계산해줘.
근무기간: 2023.03.01 ~ 2026.02.15
최근 3개월 급여: 350만원, 360만원, 355만원
상여금: 연 400%
결과를 퇴직금_계산서.md로 저장해줘" --allowedTools "Edit,Write,Bash"

# 모든 결과를 종합 보고서로
claude -p "이 폴더의 사건분석.md, 이유서_초안.md, 판례카드.md, 퇴직금_계산서.md를
하나의 종합 보고서로 통합해줘" > 종합보고서.md`}
      </CodeBlock>

      <BeforeAfter
        scenario="부당해고 구제신청 사건 처리"
        before="서류 검토 2시간 + 법령 대조 1시간 + 판례 검색 3시간 + 이유서 작성 4시간 + 퇴직금 계산 30분 = 약 2일"
        after="Claude Code로 서류 분석·법령 대조·판례 검색·이유서 초안·퇴직금 계산까지 = 약 2시간. 나머지 시간은 검토와 전략 수립에 집중."
      ...
      <KeyPoint>
        ...
        <ul className="space-y-1">
          <li><strong>Phase 1</strong> (시작) → 사건 폴더에서 Claude 실행, 문서 읽기</li>
          <li><strong>Phase 2</strong> (문서) → 취업규칙 분석, 의견서 초안 작성</li>
          <li><strong>Phase 3</strong> (CLAUDE.md) → 사건별 규칙 설정으로 품질 보장</li>
          <li><strong>Phase 4</strong> (스킬) → /사건분석 스킬로 반복 작업 자동화</li>
          <li><strong>Phase 5</strong> (MCP) → 판례 검색 자동화</li>
          <li><strong>Phase 6</strong> (자동화) → 배치 처리, 보고서 통합</li>
        </ul>
        <p className="mt-2">
...
4. 4개 결과를 교차검증 → 최종 의견서 작성`}</CodeBlock>
      ...
      <h2>실제 실행 과정</h2>
      <p>
        Claude에게 &quot;부당해고 관련 판례를 검색해줘&quot;라고 하면, 이런 과정이 자동으로 진행됩니다.
      </p>
      <CodeBlock language="bash">{`# Claude에게 법률 검토를 요청하는 예시
> 이 사건에 대해 법률 검토해줘:
> - 근로자 A가 2024.03 해고됨
> - 해고 사유: 업무 태만
> - 해고 예고 없이 즉시 해고
...
# Claude가 자동으로:
# 1. Chrome MCP로 4개 탭 오픈
# 2. 엘박스/슈퍼로이어에서 "해고예고 미이행" 검색
# 3. BigCase에서 대법원 판례 확인
# 4. 결과를 마크다운으로 정리`}</CodeBlock>
...
for i, post_path in enumerate(posts):
    ...
    if i < len(posts) - 1:
      ...
      <p>
        ...
        &quot;이 주제로 블로그 글 써줘&quot; → 마크다운 생성 → 자동 발행.
        물론 발행 전에 검토는 직접 합니다.
      </p>
      <CodeBlock language="bash">{`# Claude에게 블로그 글 작성 + 발행 요청
> "퇴직금 중간정산 요건 변경"에 대한 블로그 글을 써줘.
> 제목은 SEO 최적화해서, 본문은 1500자 내외로.
> posts/ 폴더에 마크다운으로 저장해줘.

# 이후 발행 스크립트 실행
python publish.py posts/퇴직금_중간정산_요건.md`}</CodeBlock>
    ...
    st.success("5종 서류가 생성되었습니다!")`}</CodeBlock>
      ...
      <TryThis>
        ...
        <ol className="list-decimal list-inside space-y-2 mt-2">
          ...
          <li>Streamlit 공식 문서를 확인해 봅니다 (pip install streamlit → streamlit run app.py)</li>
